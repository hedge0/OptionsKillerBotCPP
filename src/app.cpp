...
void perform_option_interpolation(risk_free_rate_val& risk_free_rate, const std::string& ticker, const std::string& date, const std::string& option_type, double min_overpriced,
	double min_underpriced, double min_oi) {
	...
	std::vector<double> filtered_strikes = filters::filter_strikes(strikes, S, 1.25);
	...
	filtered_data = filters::filter_by_bid_price(filtered_data);

	for (auto& pair: filtered_data) {
		double K		= pair.first;
		QuoteData& data = pair.second;

		data.mid_IV = models::calculate_implied_volatility_baw(data.mid, S, K, risk_free_rate.risk_free_rate, T, q, option_type);
		data.bid_IV = models::calculate_implied_volatility_baw(data.bid, S, K, risk_free_rate.risk_free_rate, T, q, option_type);
		data.ask_IV = models::calculate_implied_volatility_baw(data.ask, S, K, risk_free_rate.risk_free_rate, T, q, option_type);
	}

	filtered_data = filters::filter_by_mid_iv(filtered_data);
	...
	if (filtered_strikes.size() >= 20) {
		...
		auto interpolator		   = interpolations::rbf_model(log_x_normalized_eigen, mid_iv_eigen, 0.5);
		Eigen::VectorXd rfv_params = interpolations::fit_model(x_normalized_eigen, mid_iv_eigen, bid_iv_eigen, ask_iv_eigen);
		...
		Eigen::VectorXd rfv_interpolated_y	  = interpolations::rfv_model(log_fine_x_normalized, rfv_params);
		...
		if (filtered_x_eigen.size() >= 2) {
			...
			for (Eigen::Index i = 0; i < filtered_x_eigen.size(); ++i) {
				...
				double option_price	   = models::barone_adesi_whaley_american_option_price(S, strike, T, risk_free_rate.risk_free_rate, interpolated_iv, q, option_type);
			...
			for (Eigen::Index i = 0; i < filtered_x_eigen.size(); ++i) {
				std::cout << "Strike: " << filtered_x_eigen[i] << ", Mid Price: " << filtered_mid_eigen[i] << ", Mispricing: " << mispricings[i] << std::endl;
			}
 ...
 */
int main() {
	https_client client{ "TESTING" };
	https_workload_data data{};
	std::string newString{};
	client.submitWorkloadAndGetResult(data, newString);
	environment environment{ ".env" };
	risk_free_rate_val risk_free_rate{};

	if (environment.schwab_api_key.empty() || environment.schwab_secret.empty() || environment.callback_url.empty() || environment.account_hash.empty() ||
		environment.fred_api_key.empty()) {
	...
	risk_free_rate.fetch_risk_free_rate(environment.fred_api_key);
	...
	while (true) {
		if (is_nyse_open() || environment.dry_run) {
			perform_option_interpolation(risk_free_rate, current_node->ticker, std::to_string(current_node->date), current_node->option_type, current_node->min_overpriced,
				current_node->min_underpriced, current_node->min_oi);
		...
		std::this_thread::sleep_for(std::chrono::milliseconds(environment.time_to_rest));
		break;
	}
