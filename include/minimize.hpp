...
struct MinimizeResult {
	...
	*/
	OPTS_BOT_ALWAYS_INLINE MinimizeResult(const std::function<double(const Eigen::VectorXd&, Eigen::VectorXd&)>& func_grad, const Eigen::VectorXd& x0,
		const std::vector<std::pair<double, double>>& bounds, int maxiter = 15000, double ftol = 1e-8, double gtol = 1e-5) {
