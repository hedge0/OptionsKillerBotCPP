...
class risk_free_rate_val {
  public:
	...
	*/
	OPTS_BOT_ALWAYS_INLINE static size_t WriteCallback(void* contents, size_t size, size_t nmemb, std::string* s) {
	...
	*/
	OPTS_BOT_ALWAYS_INLINE void fetch_risk_free_rate(const std::string& api_key) {
		CURL* curl;
		CURLcode res;
		...
		if (curl) {
			...
			if (res != CURLE_OK) {
				...
				try {
					risk_free_rate_data data{};
					auto result = parser.parseJson(data, readBuffer);
					if (result) {
						risk_free_rate = data.observations.value / 100;
					}
