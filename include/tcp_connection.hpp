...
OPTS_BOT_ALWAYS_INLINE jsonifier::string reportSSLError(std::string_view errorPosition, int32_t errorValue = 0, SSL* SSL = nullptr) {
...
OPTS_BOT_ALWAYS_INLINE jsonifier::string reportError(std::string_view errorPosition) {
	std::stringstream stream{};
	stream << errorPosition << " error: ";
#if defined(_WIN32)
	#if defined(UNICODE)
...
template<typename value_type> class ssl_data_interface {
  public:
	...
	template<typename value_type_new> OPTS_BOT_ALWAYS_INLINE void writeData(jsonifier::string_view_base<value_type_new> dataToWrite, bool priority) {
...
template<typename value_type> class tcp_connection : public ssl_data_interface<tcp_connection<value_type>> {
  public:
	...
	OPTS_BOT_ALWAYS_INLINE tcp_connection(const jsonifier::string& baseUrlNew, const uint16_t portNew) {
