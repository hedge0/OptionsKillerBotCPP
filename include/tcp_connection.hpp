...
#include <openssl/ssl.h>
	...
	#include <WinSock2.h>
	#include <ws2tcpip.h>
OPTS_BOT_ALWAYS_INLINE bool isValidSocket(SOCKET s) {
...
enum class connection_status { NO_Error = 0, CONNECTION_Error = 1, POLLHUP_Error = 2, POLLNVAL_Error = 3, POLLERR_Error = 4, READ_Error = 5, WRITE_Error = 6, SOCKET_Error = 7 };

OPTS_BOT_ALWAYS_INLINE std::string reportSSLError(std::string_view errorPosition, int32_t errorValue = 0, SSL* SSL = nullptr) {
...
OPTS_BOT_ALWAYS_INLINE std::string reportError(std::string_view errorPosition) {
	std::stringstream stream{};
	stream << errorPosition << " error: ";
#if defined(_WIN32)
	#if defined(UNICODE)
	std::unique_ptr<wchar_t[]> string{ std::make_unique<wchar_t[]>(1024) };
	#else
...
struct wsadata_wrapper {
	struct wsadata_deleter {
		OPTS_BOT_ALWAYS_INLINE void operator()(WSADATA* other) {
			WSACleanup();
			delete other;
		}
	};

	OPTS_BOT_ALWAYS_INLINE wsadata_wrapper() {
		auto returnData = WSAStartup(MAKEWORD(2, 2), ptr.get());
		if (returnData) {
			std::cerr << "wsadata_wrapper::wsadata_wrapper()" << std::endl;
		}
	}

  protected:
	std::unique_ptr<WSADATA, wsadata_deleter> ptr{ new WSADATA{} };
};
#endif
...
struct ssl_ctx_wrapper {
	struct ssl_ctx_deleter {
		OPTS_BOT_ALWAYS_INLINE void operator()(SSL_CTX* other) {
			if (other) {
				SSL_CTX_free(other);
				other = nullptr;
			}
	...
	OPTS_BOT_ALWAYS_INLINE ssl_ctx_wrapper& operator=(SSL_CTX* other) {
		ptr.reset(other);
		return *this;
	}

	OPTS_BOT_ALWAYS_INLINE operator SSL_CTX*() {
		return ptr.get();
	}

  protected:
	std::unique_ptr<SSL_CTX, ssl_ctx_deleter> ptr{};
};

class ssl_wrapper {
  public:
	struct ssl_deleter {
		OPTS_BOT_ALWAYS_INLINE void operator()(SSL* other) {
			if (other) {
				SSL_shutdown(other);
				SSL_free(other);
				other = nullptr;
	...
	OPTS_BOT_ALWAYS_INLINE ssl_wrapper() = default;

	OPTS_BOT_ALWAYS_INLINE ssl_wrapper& operator=(ssl_wrapper&& other) noexcept {
		ptr = std::move(other.ptr);
		return *this;
	}

	OPTS_BOT_ALWAYS_INLINE ssl_wrapper(ssl_wrapper&& other) noexcept {
		*this = std::move(other);
	}

	OPTS_BOT_ALWAYS_INLINE ssl_wrapper& operator=(SSL* other) {
		ptr.reset(other);
		return *this;
	}

	OPTS_BOT_ALWAYS_INLINE explicit operator bool() {
		return ptr.operator bool();
	}

	OPTS_BOT_ALWAYS_INLINE operator SSL*() {
		return ptr.get();
	}

  protected:
	std::unique_ptr<SSL, ssl_deleter> ptr{};
};

class socket_wrapper {
  public:
	struct socket_deleter {
		OPTS_BOT_ALWAYS_INLINE void operator()(SOCKET* ptrNew) {
	...
	OPTS_BOT_ALWAYS_INLINE socket_wrapper() = default;

	OPTS_BOT_ALWAYS_INLINE socket_wrapper& operator=(socket_wrapper&& other) noexcept {
		ptr = std::move(other.ptr);
		return *this;
	}

	OPTS_BOT_ALWAYS_INLINE socket_wrapper(socket_wrapper&& other) noexcept {
		*this = std::move(other);
	}

	OPTS_BOT_ALWAYS_INLINE socket_wrapper& operator=(SOCKET other) {
		ptr.reset(new SOCKET{ other });
		return *this;
	}

	OPTS_BOT_ALWAYS_INLINE socket_wrapper(SOCKET other) {
		*this = other;
	}
	...
	OPTS_BOT_ALWAYS_INLINE operator SOCKET() {
  ...
  protected:
	std::unique_ptr<SOCKET, socket_deleter> ptr{};
};

struct addrinfo_wrapper {
	OPTS_BOT_ALWAYS_INLINE addrinfo* operator->() {
		return ptr;
	}

	OPTS_BOT_ALWAYS_INLINE operator addrinfo**() {
		return &ptr;
	}
...
class ssl_context_holder {
  public:
	OPTS_BOT_ALWAYS_INLINE static ssl_ctx_wrapper context;
	OPTS_BOT_ALWAYS_INLINE static std::mutex accessMutex;

	OPTS_BOT_ALWAYS_INLINE static bool initialize() {
...
template<typename value_type> class ssl_data_interface {
  public:
	...
	template<typename value_type_new> OPTS_BOT_ALWAYS_INLINE void writeData(std::basic_string_view<value_type_new> dataToWrite, bool priority) {
	...
	OPTS_BOT_ALWAYS_INLINE auto getInputBuffer() {
		return inputBuffer.readData();
	}

	OPTS_BOT_ALWAYS_INLINE int64_t getBytesRead() {
		return bytesRead;
	}

	OPTS_BOT_ALWAYS_INLINE void reset() {
		outputBuffer.clear();
		inputBuffer.clear();
		bytesRead = 0;
	}
  ...
  protected:
	...
	OPTS_BOT_ALWAYS_INLINE ssl_data_interface() = default;
...
template<typename value_type> class tcp_connection : public ssl_data_interface<tcp_connection<value_type>> {
  public:
	...
	OPTS_BOT_ALWAYS_INLINE tcp_connection(const std::string& baseUrlNew, const uint16_t portNew) {
		...
		if (httpsFind != std::string::npos && comFind != std::string::npos) {
			addressString =
				baseUrlNew.substr(httpsFind + std::string_view{ "https://" }.size(), comFind + std::string_view{ ".com" }.size() - std::string_view{ "https://" }.size());
		} else if (httpsFind != std::string::npos && orgFind != std::string::npos) {
			addressString =
				baseUrlNew.substr(httpsFind + std::string_view{ "https://" }.size(), orgFind + std::string_view{ ".org" }.size() - std::string_view{ "https://" }.size());
		} else {
		...
		if (getaddrinfo(addressString.data(), jsonifier::toString(portNew).data(), hints, address)) {
			std::cerr << "Tcp_connection::getaddrinfo(), to: " << baseUrlNew << std::endl;
		...
		if (socket = ::socket(address->ai_family, address->ai_socktype, address->ai_protocol); !isValidSocket(socket.operator SOCKET())) {
			std::cerr << "Tcp_connection::SOCKET(), to: " << baseUrlNew << std::endl;
			currentStatus = connection_status::CONNECTION_Error;
			socket		  = INVALID_SOCKET;
			return;
		}
		...
		if (::connect(socket, address->ai_addr, static_cast<int32_t>(address->ai_addrlen)) == SOCKET_ERROR) {
			std::cerr << "Tcp_connection::connect(), to: " << baseUrlNew << std::endl;
			currentStatus = connection_status::CONNECTION_Error;
			socket		  = INVALID_SOCKET;
			return;
		}
		...
		if (ssl = SSL_new(ssl_context_holder::context); !ssl) {
			std::cout << reportSSLError("Tcp_connection::connect::SSL_new(), to: " + baseUrlNew) + "\n" + reportError("Tcp_connection::connect::SSL_new(), to: " + baseUrlNew)
					  << std::endl;
			currentStatus = connection_status::CONNECTION_Error;
			socket		  = INVALID_SOCKET;
			ssl			  = nullptr;
		...
		if (auto result{ SSL_set_fd(ssl, static_cast<int32_t>(socket)) }; result != 1) {
			std::cout << reportSSLError("Tcp_connection::connect::SSL_set_fd(), to: " + baseUrlNew) + "\n" +
		...
		if (auto result{ SSL_set_tlsext_host_name(ssl, addressString.data()) }; result != 1) {
			std::cout << reportSSLError("Tcp_connection::connect::SSL_set_tlsext_host_name(), to: " + baseUrlNew) + "\n" +
		...
		if (auto result{ SSL_connect(ssl) }; result != 1) {
			std::cout << reportSSLError("Tcp_connection::connect::SSL_connect(), to: " + baseUrlNew) + "\n" +
		...
		if (auto returnData{ ioctlsocket(socket, FIONBIO, &value02) }; returnData == SOCKET_ERROR) {
			std::cerr << "Tcp_connection::connect::ioctlsocket(), to: " << baseUrlNew << std::endl;
			currentStatus = connection_status::CONNECTION_Error;
			socket		  = INVALID_SOCKET;
			ssl			  = nullptr;
			return;
		...
		if (auto returnData{ fcntl(socket, F_SETFL, fcntl(socket, F_GETFL, 0) | O_NONBLOCK) }; returnData == SOCKET_ERROR) {
			std::cerr << "Tcp_connection::connect::fcntl(), to: " << baseUrlNew << std::endl;
			currentStatus = connection_status::CONNECTION_Error;
			socket		  = INVALID_SOCKET;
			ssl			  = nullptr;
			return;
	...
	OPTS_BOT_ALWAYS_INLINE connection_status processIO(int32_t waitTimeInMs) {
		...
		if (auto returnValue = poll(&readWriteSet, 1, waitTimeInMs); returnValue == SOCKET_ERROR) {
			std::cout << reportSSLError("Tcp_connection::processIO() 00") + "\n" + reportError("Tcp_connection::processIO() 00") << std::endl;
			socket		  = INVALID_SOCKET;
			ssl			  = nullptr;
			...
			if (readWriteSet.revents & POLLOUT || (POLLIN && writeWantRead)) {
				if (!processWriteData()) {
					std::cout << reportSSLError("Tcp_connection::processIO() 01") + "\n" + reportError("Tcp_connection::processIO() 01") << std::endl;
			...
			if (readWriteSet.revents & POLLIN || (POLLOUT && readWantWrite)) {
				if (!processReadData()) {
					std::cout << reportSSLError("Tcp_connection::processIO() 02") + "\n" + reportError("Tcp_connection::processIO() 02") << std::endl;
			...
			if (readWriteSet.revents & POLLERR) {
				std::cout << reportSSLError("Tcp_connection::processIO() 03") + "\n" + reportError("Tcp_connection::processIO() 03") << std::endl;
				currentStatus = connection_status::POLLERR_Error;
				socket		  = INVALID_SOCKET;
				ssl			  = nullptr;
			}
			if (readWriteSet.revents & POLLNVAL) {
				std::cout << reportSSLError("Tcp_connection::processIO() 04") + "\n" + reportError("Tcp_connection::processIO() 04") << std::endl;
	...
	OPTS_BOT_ALWAYS_INLINE bool areWeStillConnected() {
	...
	OPTS_BOT_ALWAYS_INLINE bool processWriteData() {
		writeWantRead  = false;
	...
	OPTS_BOT_ALWAYS_INLINE bool processReadData() {
		readWantRead  = false;
		readWantWrite = false;
	...
	template<typename value_type2> OPTS_BOT_ALWAYS_INLINE static std::unordered_map<uint64_t, value_type2*> processIO(std::unordered_map<uint64_t, value_type2*>& shardMap) {
		std::unordered_map<uint64_t, value_type2*> returnData{};
	...
	virtual OPTS_BOT_ALWAYS_INLINE void handleBuffer() = 0;

	OPTS_BOT_ALWAYS_INLINE void disconnect() {
		currentStatus = connection_status::CONNECTION_Error;
	...
	virtual OPTS_BOT_ALWAYS_INLINE ~tcp_connection() = default;

  protected:
	OPTS_BOT_ALWAYS_INLINE tcp_connection() = default;
};
